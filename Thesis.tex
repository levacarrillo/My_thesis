...
El error en la medición $z \in \Re^m$ se puede modelar con la ecuación \ref{eq:measurement_equation}:
\begin{equation}
z_k = Hx_k+v_k
\label{eq:measurement_equation}
\end{equation}
$H$ es una matriz de $mxn$ que relaciona el estado con la medición $z_k$.  Las variables aleatorias $w_k$ y $v_k$ representan el ruido del proceso y de la medición (respectivamente), asumiendo que son independientes una de la otra. Se dice que \ref{eq:kalman_filter} es una ecuación lineal \textit{estocástica} debido a que los datos adquiridos tienen un comportamiento no determinista.
